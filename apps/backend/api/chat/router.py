...
def _normalize_user_message(message: str, language_hint: Optional[str] = None) -> tuple[str, Dict[str, Any]]:
    ...
    detected_language = (language_hint or detect_language(normalized)).lower()
    
    # Spoken Urdu is often transcribed as Hindi (Devanagari script) by Whisper. Treat it as Urdu.
    input_language = "ur" if detected_language == "hi" else detected_language
    ...
    if input_language == "ur":
        output_message, translated, translation_error = translate_to_english(normalized, source_lang=detected_language)
...
async def send_audio_message(
    ...
    try:
        ...
        # If the ASR generated Hindi (Devanagari), seamlessly rewrite the transcript displayed on UI to Urdu (Nastaliq)
        display_transcript = transcript
        if language_metadata.get("original_detected_language") == "hi":
            from services.asr.translation import translate_to_urdu
            urdu_text, did_trans, _ = translate_to_urdu(normalized_message, source_lang="en")
            if did_trans and urdu_text:
                display_transcript = urdu_text
