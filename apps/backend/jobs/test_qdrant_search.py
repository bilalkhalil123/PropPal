"""
Test script to verify Qdrant search is working correctly.
"""
import asyncio
from services.vector_search.qdrant_service import search_properties
from services.embeddings.service import embed_text
from common.qdrant import get_qdrant_client, PROPERTIES_COLLECTION


async def test_search():
    """Test Qdrant search functionality."""
    print("[TEST] Testing Qdrant search...")
    
    # Check if collection exists and has data
    client = get_qdrant_client()
    try:
        collection_info = client.get_collection(PROPERTIES_COLLECTION)
        print(f"[TEST] Collection '{PROPERTIES_COLLECTION}' exists")
    ...
    results = await search_properties(query_vec, limit=5)
